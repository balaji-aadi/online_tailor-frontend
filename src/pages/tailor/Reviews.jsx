...
import { Star, Eye, MessageSquare, Search, Package, Users, Trash2 } from 'lucide-react';
...
const TailorReviews = () => {
    ...
    const fetchReviews = async () => {
        ...
        try {
            ...
            const response = activeTab === 'service'
                ? await ReviewApi.getTailorServiceReviews(params)
                : await ReviewApi.getTailorReviews(params);
    ...
    const handleViewDetails = async (review) => {
        try {
            const response = await ReviewApi.getTailorReviewDetails(review._id);
    ...
    const handleSubmitReply = async () => {
        try {
            await ReviewApi.replyToTailorReview(selectedReview._id, { reply: tailorReply });
    ...
    const tailorColumns = [
        ...
        {
            header: "Reply",
            accessorKey: 'tailorReply',
            cell: ({ row }) => (
                <Description description={row.original.tailorReply || "Not reply"} length={50} />
            )
        },
        {
            header: t.actions,
                ...
                >
                    {selectedReview && (
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700">{t.reviewContent}</label>
                                <p className="mt-1 text-sm text-gray-900">{selectedReview.review || 'N/A'}</p>
                            </div>
                            ...
                            {selectedReview.tailorReply && (
                                <div>
                                    <label className="block text-sm font-medium text-gray-700">{t.yourReply}</label>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <textarea
                                    ...
                                    defaultValue={selectedReview.tailorReply || ''}
                                />
                            </div>
                        </div>
                    )}
